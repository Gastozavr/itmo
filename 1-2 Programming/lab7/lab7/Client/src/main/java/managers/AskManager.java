...
public class AskManager implements Serializable {
    ...
    public static StudyGroup askStudyGroup(Console console, Long id,String login) throws AskBreak {
        try {
            String name;
            while (true) {
                console.print("name: ");
                ...
                if (name.contains(";")) console.println("Имя содержит запрещенные символы");
                if (!name.isEmpty() & !name.contains(";")) break;
            }
            ...
            return new StudyGroup(id, name, coordinates, LocalDate.now(), studentsCount, formOfEducation, semester,login, admin);
