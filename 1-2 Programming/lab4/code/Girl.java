...
import prog.lab4.SayException;
import prog.lab4.Sayable;
...
public class Girl extends Person implements Sayable {
    ...
    public void shakeHead() {
        System.out.println(this.name + " покачала головой");
    }

    public void put(Weariable object) throws ItemsException {

        if (this.items.size() == 1 && !this.items.contains(object)) {
            throw new ItemsException("У " + this.name + " нет такого объекта");
        }
        if (object == Weariable.NOTHING) {
            throw new ItemsException(this.name + " пытается поставить ничего");
        }

        System.out.println(this.name + " поставила " + object.Type());
    ...
    public void say(String message) {
        if (message == "") {
            throw new SayException(this.name + " ничего не говорит");
        }
        System.out.println(this.name + " сказала  \"" + message + "\"");
    }
    ...
    public List<Jowls.JowlsTypes> getJowlsTypes() {
        return this.jowls.getType();
    }

    public void setJowlsTypes(Jowls.JowlsTypes... types) {
